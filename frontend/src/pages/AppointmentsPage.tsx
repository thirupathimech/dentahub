...
export default function AppointmentsPage() {
  ...
  return <div className="space-y-6 py-7">
    ...
    {confirmOpen && <ConfirmationModal editing={editing} patientName={patients.find((patient) => String(patient.id) === form.patientId)?.fullName ?? 'Selected patient'} doctorName={doctors.find((doctor) => String(doctor.id) === form.doctorId)?.fullName ?? 'Selected doctor'} form={form} saving={saving} onClose={() => setConfirmOpen(false)} onConfirm={() => confirmSave(false)} />}
