...
export default function UsersRolesPage() {
  ...
  const openRoleEdit = (role: Role) => { setEditingRole(role); setRoleForm({ name: role.name, description: role.description ?? '', permissions: role.permissions?.split(',').filter(Boolean) ?? [], active: role.active }); setModal('role'); setError('') }
  ...
  async function saveRole(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setError('')
    try {
      const saved = editingRole ? await apiPut<Role>(`/api/roles/${editingRole.id}`, { ...roleForm, permissions: roleForm.permissions.join(',') }) : await apiPost<Role>('/api/roles', { ...roleForm, permissions: roleForm.permissions.join(',') })
...
function RoleModal({ editing, form, setForm, saving, error, onClose, onSave }: { editing: Role | null; form: RoleForm; setForm: (form: RoleForm) => void; saving: boolean; error: string; onClose: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-ink/30 p-0 backdrop-blur-sm sm:items-center sm:p-5"><div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl"><div className="flex items-start justify-between"><div><h2 className="heading-font text-xl font-extrabold text-ink">{editing ? 'Edit role' : 'Add role'}</h2><p className="mt-1 text-xs text-muted">Choose the menu areas this role can access.</p></div><button onClick={onClose} className="rounded-lg p-2 text-muted hover:bg-slate-100"><X size={18} /></button></div><form onSubmit={onSave} className="mt-6 space-y-4"><Field label="Role name" required value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><TextArea label="Description" value={form.description} onChange={(value) => setForm({ ...form, description: value })} /><div><p className="mb-2 text-xs font-bold text-ink">Menu permissions</p><div className="grid gap-2 sm:grid-cols-2">{MENU_PERMISSIONS.map((permission) => <label key={permission.key} className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-semibold text-ink transition hover:border-teal-200 hover:bg-teal-50/40"><input type="checkbox" checked={form.permissions.includes(permission.key)} onChange={(event) => setForm({ ...form, permissions: event.target.checked ? [...form.permissions, permission.key] : form.permissions.filter((key) => key !== permission.key) })} className="h-4 w-4 accent-teal-600" /><span>{permission.label}</span></label>)}</div></div><label className="flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-3 text-xs font-bold text-ink"><input type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} className="h-4 w-4 accent-teal-600" />Role is active</label>{error && <p className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-600">{error}</p>}<div className="flex justify-end gap-3 border-t border-slate-100 pt-5"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-xs font-bold text-muted hover:bg-slate-100">Cancel</button><button disabled={saving} className="rounded-xl bg-teal-600 px-5 py-2.5 text-xs font-bold text-white hover:bg-teal-700 disabled:opacity-60">{saving ? 'Saving...' : editing ? 'Save changes' : 'Create role'}</button></div></form></div></div>
}
